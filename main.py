...
def train_network(sess):
    ...
    while current_epoch <= params.epochs:
        ...
        if current_epoch % params.training_display == 0:
            #training_accuracy, training_loss, predictions, costs = sess.run([accuracy, cost, prediction_round_values, cost_examination], feed_dict={x: mini_batch_x, y: mini_batch_y, keep_prob: params.dropout})
...
def test_network(sess):
    ...
    while start_index < total_elements:
        ...
        test_data_real = data_set.load_image_data(test_data, start_index, end_index)
        test_labels_real = data_set.load_label_datafile(test_labels, start_index, end_index)
